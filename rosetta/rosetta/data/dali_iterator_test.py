...
eval_index_path = '/opt/rosetta/datasets/imagenet/index/eval_idxs/'

testing_urls = '/opt/rosetta/dev/dataset/dataset.tar'
testing_index = '/opt/rosetta/dev/dataset/index/'
...
def test_run_dali_dataset_configured_for_vit():
...
def test_run_dali_iterator_for_vit():
  iterator = dali_iterator.get_dali_dataset(
    config, ds_shard_id, num_ds_shards, None, total_steps, image_shape, num_classes, is_training)
  
  for _ in range(1001):
    batch = next(iterator)
    # is_nonpadding = iterator.is_nonpadding
    
    # print(is_nonpadding, type(is_nonpadding), is_nonpadding.shape)
    
    
def test_compare_dataset_and_iterator_output():
  iterator_old = get_dali_dataset_configured(
    config, image_shape, num_classes, ds_shard_id, num_ds_shards)
  
  iterator_new = dali_iterator.get_dali_dataset(
      config, ds_shard_id, num_ds_shards, None, total_steps, image_shape, num_classes, is_training)
  
  for _ in range(1000):
    batch_old = next(iterator_old)
    batch_new = next(iterator_new)
    
    for key in batch_old.keys():
      # print(key)
      # print(batch_old[key].shape)
      # print(batch_new[key].shape)
      # print(jnp.allclose(batch_old[key], batch_new[key]))
      # print()
      
      assert jnp.allclose(batch_old[key], batch_new[key])
